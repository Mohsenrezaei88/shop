...
Dropzone.autoDiscover = false; // مهم: تا Dropzone خودش به صورت اتومات فعال نشه

Dropzone.autoDiscover = false; // جلوگیری از init خودکار قالب

const myDropzone = new Dropzone("#my-dropzone", {
    url: uploadUrl, // متغیری که در Blade تعریف کردیم
    paramName: "file", // نام فیلد فایل
    maxFilesize: 5, // مگابایت
    autoProcessQueue: true, // ارسال لحظه‌ای
    addRemoveLinks: true, // لینک حذف فایل
    maxFiles: 20,
    clickable: true, // اجازه کلیک برای انتخاب فایل
    headers: {
        "X-CSRF-TOKEN": csrf, // CSRF لاراول
    },
});
myDropzone.on("addedfile", function (file) {
    const minFiles = 2; // حداقل تعداد فایل
    if (myDropzone.files.length < minFiles) {
        // console.warn("حداقل " + minFiles + " فایل باید اضافه شود.");
        imageLable.textContent = "*حداقل" + minFiles + " فایل باید اضافه شود.";
    } else {
        imageLable.textContent = "";
    }
});

// Event برای نمایش فایل اضافه شده
// myDropzone.on("addedfile", (file) => {
//     console.log("فایل اضافه شد:", file.name);
//     alert("فایل اضافه شد: " + file.name);
// });
...
myDropzone.on("error", function (file, response, xhr) {
    if (xhr && xhr.status === 409) {
        alert("فایل وارد شده قبلا ذخیره شده");
    }
...
myDropzone.on("removedfile", function (file) {
    if (!file.isMock) {
        fetch(deleteUrl, {
            ...
            body: JSON.stringify({ image: file.upload.filename  , new : 1}),
        })
            .then((res) => res.json())
...
if (files) {
    files.forEach((file) => {
        let mockFile = { name: file.name, size: file.size, url: file.url };
        myDropzone.emit("addedfile", mockFile);
        myDropzone.emit(
            "thumbnail",
            mockFile,
            "http://127.0.0.1:8000/" + file.url
        );
